/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:30:53 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/Frameworks/ModelIO.framework/ModelIO
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/


#import <ModelIO/ModelIO-Structs.h>
@interface MDLVolumeGrid : NSObject {

	unique_ptr<ModelIO::Octree, std::__1::default_delete<ModelIO::Octree> >* _octree;

}
-(id)initWithAsset:(id)arg1 divisions:(int)arg2 ;
-(id)initWithAsset:(id)arg1 divisions:(int)arg2 interiorWidth:(float)arg3 exteriorWidth:(float)arg4 patchRadius:(float)arg5 ;
-(id)meshWithStyle:(unsigned)arg1 ;
-(id)YZlayerAtX:(float)arg1 forLevel:(int)arg2 ;
-(id)XZlayerAtY:(float)arg1 forLevel:(int)arg2 ;
-(id)XYlayerAtZ:(float)arg1 forLevel:(int)arg2 ;
-(id)createVertexBufferWithStyle:(unsigned)arg1 quality:(float)arg2 ;
-(id)createIndexBufferWithStyle:(unsigned)arg1 quality:(float)arg2 ;
-(id)createVertexBufferWithStyle:(unsigned)arg1 quality:(float)arg2 exteriorWidth:(float)arg3 interiorWidth:(float)arg4 ;
-(id)createIndexBufferWithStyle:(unsigned)arg1 quality:(float)arg2 exteriorWidth:(float)arg3 interiorWidth:(float)arg4 ;
-(void)removeSelfIntersections;
-(void)expandBoundaryWithExteriorWidth:(float)arg1 interiorWidth:(float)arg2 ;
-(void)contractBoundaryWithExteriorWidth:(float)arg1 interiorWidth:(float)arg2 ;
-(void)resampleGrid:(float)arg1 ;
-(void)filterBy:(/*^block*/id)arg1 ;
-(8)gridCellExtent;
-(8)gridExtent;
-(float)sampleAtIndex:;
-(void)setSampleAtIndex:(float)arg1 ;
@end

