/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:30:12 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/Frameworks/AddressBookUI.framework/AddressBookUI
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/


#import <AddressBookUI/AddressBookUI-Structs.h>
@class NSMutableArray;

@interface ABPersonTableActionDataSource : NSObject {

	NSMutableArray* _topActions;
	NSMutableArray* _bottomActions;
	CFDictionaryRef _actionsByProperty;
	CFDictionaryRef _actionGroupingsByProperty;
	CFDictionaryRef _inlineActionsByProperty;
	CFSetRef _hiddenActions;

}
-(char)hasActionWithTarget:(id)arg1 selector:(SEL)arg2 property:(int)arg3 actionGrouping:(int)arg4 ordering:(int)arg5 outActions:(id*)arg6 outIndex:(int*)arg7 ;
-(id)topActions;
-(id)bottomActions;
-(CFDictionaryRef)inlineActionsByProperty;
-(CFDictionaryRef)actionsByProperty;
-(CFDictionaryRef)actionGroupingsByProperty;
-(id)actionsForProperty:(int)arg1 withActionGrouping:(int)arg2 ;
-(char)hasActionWithTarget:(id)arg1 selector:(SEL)arg2 matchProperty:(char)arg3 property:(int)arg4 inArray:(id)arg5 outActions:(id*)arg6 outIndex:(int*)arg7 ;
-(void)removeActionWithTarget:(id)arg1 selector:(SEL)arg2 forProperty:(int)arg3 withActionGrouping:(int)arg4 ordering:(int)arg5 ;
-(CFArrayRef)groupingsForProperty:(int)arg1 ;
-(id)addActionWithTitle:(id)arg1 shortTitle:(id)arg2 detailText:(id)arg3 style:(int)arg4 target:(id)arg5 selector:(SEL)arg6 property:(int)arg7 actionGrouping:(int)arg8 ordering:(int)arg9 ;
-(void)removeAction:(id)arg1 ;
-(id)newActionsForProperty:(int)arg1 ;
-(int)groupingCountForProperty:(int)arg1 ;
-(int)actionCountForProperty:(int)arg1 withActionGrouping:(int)arg2 ;
-(id)actionForProperty:(int)arg1 withActionGrouping:(int)arg2 atIndex:(int)arg3 ;
-(id)actionsForProperty:(int)arg1 withActionGroupingAtIndex:(int)arg2 ;
-(int)actionCountForActionGrouping:(int)arg1 ;
-(CFArrayRef)indexesOfActionsForProperty:(int)arg1 withActionGrouping:(int)arg2 ;
-(id)actionWithActionGrouping:(int)arg1 atIndex:(int)arg2 ;
-(char)isActionHidden:(id)arg1 ;
-(void)setAction:(id)arg1 hidden:(char)arg2 ;
-(void)dealloc;
@end

