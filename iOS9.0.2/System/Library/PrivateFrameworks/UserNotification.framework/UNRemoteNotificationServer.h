/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:34:37 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/PrivateFrameworks/UserNotification.framework/UserNotification
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <UserNotification/UserNotification-Structs.h>
#import <libobjc.A.dylib/APSConnectionDelegate.h>
#import <libobjc.A.dylib/LSApplicationWorkspaceObserverProtocol.h>
#import <libobjc.A.dylib/FBProcessManagerObserver.h>
#import <libobjc.A.dylib/FBProcessObserver.h>

@protocol OS_dispatch_queue, UNSystemStateProviding, UNRemoteNotificationServerDelegate;
@class NSObject, BKSApplicationStateMonitor, LSApplicationWorkspace, NSMutableDictionary, NSMutableSet, NSString;

@interface UNRemoteNotificationServer : NSObject <APSConnectionDelegate, LSApplicationWorkspaceObserverProtocol, FBProcessManagerObserver, FBProcessObserver> {

	NSObject*<OS_dispatch_queue> _queue;
	BKSApplicationStateMonitor* _appStateMonitor;
	LSApplicationWorkspace* _appWorkspace;
	id<UNSystemStateProviding> _systemStateProvider;
	NSMutableDictionary* _bundleIdentifiersToClients;
	NSMutableDictionary* _environmentsToConnections;
	NSMutableSet* _bundleIdentifiersNeedingToken;
	NSMutableSet* _userNotificationEnabledBundleIdentifiers;
	NSMutableSet* _debuggingBundleIdentifiers;
	NSMutableSet* _foregroundBundleIdentifiers;
	NSMutableSet* _importantBundleIdentifiers;
	id<UNRemoteNotificationServerDelegate> _delegate;

}

@property (assign,nonatomic) id<UNRemoteNotificationServerDelegate> delegate;              //@synthesize delegate=_delegate - In the implementation block
@property (retain) id<UNSystemStateProviding> systemStateProvider;                         //@synthesize systemStateProvider=_systemStateProvider - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(id)_environmentFromAuditToken:(SCD_Struct_UN0*)arg1 ;
+(id)environmentFromAuditToken:(SCD_Struct_UN0*)arg1 ;
-(void)setDelegate:(id<UNRemoteNotificationServerDelegate>)arg1 ;
-(void)dealloc;
-(id<UNRemoteNotificationServerDelegate>)delegate;
-(void)connection:(id)arg1 didReceiveToken:(id)arg2 forTopic:(id)arg3 identifier:(id)arg4 ;
-(void)connection:(id)arg1 didReceivePublicToken:(id)arg2 ;
-(void)connection:(id)arg1 didReceiveIncomingMessage:(id)arg2 ;
-(void)applicationProcessDebuggingStateDidChange:(id)arg1 ;
-(void)processManager:(id)arg1 didAddProcess:(id)arg2 ;
-(void)processManager:(id)arg1 didRemoveProcess:(id)arg2 ;
-(void)applicationsDidInstall:(id)arg1 ;
-(void)applicationsDidUninstall:(id)arg1 ;
-(void)setAllowsBackgroundRefresh:(char)arg1 forBundleIdentifier:(id)arg2 ;
-(void)_userNotificationSettingsDidChange:(id)arg1 ;
-(void)_queue_appStateDidChange:(id)arg1 ;
-(void)_queue_calculateTopics;
-(void)_queue_connection:(id)arg1 didReceiveToken:(id)arg2 forTopic:(id)arg3 identifier:(id)arg4 ;
-(void)_queue_connection:(id)arg1 didReceivePublicToken:(id)arg2 ;
-(void)_queue_connection:(id)arg1 didReceiveIncomingMessage:(id)arg2 ;
-(char)_queue_isPushedEnabledForApplication:(id)arg1 ;
-(char)_queue_isNewsstandPushEnabledForApplication:(id)arg1 ;
-(char)_queue_isAppForeground:(id)arg1 ;
-(char)_queue_isNewsstandApplication:(id)arg1 ;
-(void)_queue_invalidateTokenForBundleIdentifier:(id)arg1 ;
-(void)_queue_removeClientForBundleIdentifier:(id)arg1 ;
-(void)_queue_appDebugStateDidChange:(id)arg1 ;
-(char)_queue_isBackgroundAppRefreshSupportedForApplication:(id)arg1 ;
-(char)_queue_isBackgroundAppRefreshAllowedForBundleIdentifier:(id)arg1 ;
-(id)_queue_allTopicsForApplication:(id)arg1 ;
-(char)_queue_isAppImportant:(id)arg1 ;
-(char)_queue_isAppDebugging:(id)arg1 ;
-(void)_queue_appDidBecomeImportant:(id)arg1 ;
-(void)_queue_appDidBecomeUnimportant:(id)arg1 ;
-(void)_queue_moveTopicsForApp:(id)arg1 fromList:(unsigned)arg2 toList:(unsigned)arg3 ;
-(void)_queue_appDidBecomeForeground:(id)arg1 ;
-(void)_queue_appDidBecomeBackground:(id)arg1 ;
-(void)_queue_appImportanceChanged:(id)arg1 ;
-(void)_queue_appDidGetKilledByUser:(id)arg1 ;
-(void)_queue_setBackgroundDeliveryDisabled:(char)arg1 forBundleIdentifier:(id)arg2 ;
-(char)_queue_isNewsstandContentBackgroundModeSupportedForApplication:(id)arg1 ;
-(char)_queue_isUserNotificationEnabledForApplication:(id)arg1 ;
-(void)_queue_userNotificationSettingsDidChange:(id)arg1 ;
-(void)_queue_userNotificationsChangedStateForBundleIdentifier:(id)arg1 becameEnabled:(char)arg2 ;
-(id)_queue_cloudDatabaseTopicsForApplication:(id)arg1 ;
-(void)_queue_registerApplicationWithBundleIdentifier:(id)arg1 forEnvironment:(id)arg2 appWantsPush:(char)arg3 ;
-(void)_queue_requestTokenForClient:(id)arg1 ;
-(id)_queue_backgroundAppRefreshBundleIdentifierMapping;
-(void)_queue_setBackgroundAppRefreshAllowed:(char)arg1 forBundleIdentifier:(id)arg2 ;
-(unsigned)_queue_remoteNotificationTypesForBundleIdentifier:(id)arg1 ;
-(id)initWithSystemStateProvider:(id)arg1 ;
-(char)_queue_isUserNotificationEnabled:(id)arg1 ;
-(void)requestRemoteNotificationTokenWithEnvironment:(id)arg1 forBundleIdentifier:(id)arg2 ;
-(void)invalidateTokenForRemoteNotificationsForBundleIdentifier:(id)arg1 ;
-(void)backgroundAppRefreshBundleIdentifierMappingWithResult:(/*^block*/id)arg1 ;
-(void)allowsRemoteNotificationsForBundleIdentifier:(id)arg1 withResult:(/*^block*/id)arg2 ;
-(void)remoteNotificationTypesForBundleIdentifier:(id)arg1 withResult:(/*^block*/id)arg2 ;
-(id<UNSystemStateProviding>)systemStateProvider;
-(void)setSystemStateProvider:(id<UNSystemStateProviding>)arg1 ;
@end

