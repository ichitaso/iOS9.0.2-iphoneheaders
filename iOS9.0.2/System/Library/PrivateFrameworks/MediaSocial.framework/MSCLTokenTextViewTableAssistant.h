/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:33:18 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/PrivateFrameworks/MediaSocial.framework/MediaSocial
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <MediaSocial/MediaSocial-Structs.h>
#import <libobjc.A.dylib/MSCLTagListDelegate.h>
#import <libobjc.A.dylib/NSLayoutManagerDelegate.h>
#import <UIKit/UITextViewDelegate.h>

@class MSCLConfiguration, UIViewController, MSCLTagListViewController, NSMutableOrderedSet, UITableView, NSString;

@interface MSCLTokenTextViewTableAssistant : NSObject <MSCLTagListDelegate, NSLayoutManagerDelegate, UITextViewDelegate> {

	MSCLConfiguration* _configuration;
	CGRect _keyboardFrame;
	UIViewController* _parentViewController;
	MSCLTagListViewController* _tagCompletionViewController;
	NSMutableOrderedSet* _textViews;
	UITableView* _tableView;

}

@property (assign,nonatomic,__weak) UIViewController * parentViewController;              //@synthesize parentViewController=_parentViewController - In the implementation block
@property (nonatomic,retain) UITableView * tableView;                                     //@synthesize tableView=_tableView - In the implementation block
@property (assign,nonatomic) CGRect keyboardFrame;                                        //@synthesize keyboardFrame=_keyboardFrame - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(void)dealloc;
-(UIViewController *)parentViewController;
-(UITableView *)tableView;
-(void)setTableView:(UITableView *)arg1 ;
-(void)layoutManager:(id)arg1 didCompleteLayoutForTextContainer:(id)arg2 atEnd:(char)arg3 ;
-(void)setParentViewController:(UIViewController *)arg1 ;
-(void)textViewDidBeginEditing:(id)arg1 ;
-(void)textViewDidEndEditing:(id)arg1 ;
-(char)textView:(id)arg1 shouldChangeTextInRange:(NSRange)arg2 replacementText:(id)arg3 ;
-(void)textViewDidChange:(id)arg1 ;
-(void)textViewDidChangeSelection:(id)arg1 ;
-(CGRect)keyboardFrame;
-(id)initWithConfiguration:(id)arg1 ;
-(void)addTextView:(id)arg1 ;
-(void)setKeyboardFrame:(CGRect)arg1 ;
-(void)tagList:(id)arg1 didSelectTag:(id)arg2 ;
-(id)_tagCompletionViewController;
-(id)_activeTextView;
-(id)_indexPathForTextView:(id)arg1 ;
-(void)_positionTagCompletionsBelowCellForIndexPath:(id)arg1 animated:(char)arg2 ;
-(void)_removeTagCompletionsAnimated:(char)arg1 ;
-(void)_showTagCompletionsAnimated:(char)arg1 ;
-(void)layoutTagCompletionView;
@end

