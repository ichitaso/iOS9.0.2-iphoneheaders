/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:32:41 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/PrivateFrameworks/FrontBoardServices.framework/FrontBoardServices
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <FrontBoardServices/FrontBoardServices-Structs.h>
#import <libobjc.A.dylib/BSXPCCoding.h>
#import <libobjc.A.dylib/BSSettingDescriptionProvider.h>

@protocol BSSettingDescriptionProvider;
@class BSSettingsDiff, BSSettings, NSString;

@interface FBSSettingsDiff : NSObject <BSXPCCoding, BSSettingDescriptionProvider> {

	BSSettingsDiff* _otherSettingsDiff;
	id<BSSettingDescriptionProvider> _descriptionProvider;
	BSSettings* _changes;
	BSSettings* _otherSettingsChanges;
	Class _settingsClass;

}

@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(void)dealloc;
-(id)init;
-(NSString *)description;
-(id)keyDescriptionForSetting:(unsigned)arg1 ;
-(id)initWithXPCDictionary:(id)arg1 ;
-(id)valueDescriptionForFlag:(int)arg1 object:(id)arg2 ofSetting:(unsigned)arg3 ;
-(void)encodeWithXPCDictionary:(id)arg1 ;
-(void)inspectChangesWithBlock:(/*^block*/id)arg1 ;
-(id)settings:(id)arg1 keyDescriptionForSetting:(unsigned)arg2 ;
-(id)settings:(id)arg1 valueDescriptionForFlag:(int)arg2 object:(id)arg3 ofSetting:(unsigned)arg4 ;
-(id)descriptionWithMultilinePrefix:(id)arg1 ;
-(id)succinctDescription;
-(id)succinctDescriptionBuilder;
-(id)descriptionBuilderWithMultilinePrefix:(id)arg1 ;
-(id)initWithChanges:(id)arg1 fromSettings:(id)arg2 toSettings:(id)arg3 ;
-(char)inspectSetting:(unsigned)arg1 forCGRect:(CGRect*)arg2 ;
-(char)inspectSetting:(unsigned)arg1 forNumber:(id*)arg2 ;
-(id)_initWithChanges:(id)arg1 otherSettingsDiff:(id)arg2 settingsClass:(Class)arg3 ;
-(char)_inspectSetting:(unsigned)arg1 ofSettings:(id)arg2 forObject:(id*)arg3 ofClass:(Class)arg4 ;
-(char)inspectSetting:(unsigned)arg1 forObject:(id*)arg2 ofClass:(Class)arg3 ;
-(char)_inspectSetting:(unsigned)arg1 ofSettings:(id)arg2 forCGRect:(CGRect*)arg3 ;
-(char)_inspectSetting:(unsigned)arg1 ofSettings:(id)arg2 forFlag:(int*)arg3 ;
-(char)inspectOtherSetting:(unsigned)arg1 forNumber:(id*)arg2 ;
-(id)_otherSettingsDescriptionProvider;
-(char)inspectSetting:(unsigned)arg1 forFlag:(int*)arg2 ;
-(char)inspectOtherSetting:(unsigned)arg1 forObject:(id*)arg2 ofClass:(Class)arg3 ;
-(char)inspectOtherSetting:(unsigned)arg1 forCGRect:(CGRect*)arg2 ;
-(char)inspectOtherSetting:(unsigned)arg1 forFlag:(int*)arg2 ;
-(void)inspectOtherChangesWithBlock:(/*^block*/id)arg1 ;
@end

