/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:38:16 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/PrivateFrameworks/CoreRecents.framework/recentsd
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/


@protocol CNCancelable;
@class CRMailAccountAnalyzer;

@interface CRMailAccountAdaptor : NSObject {

	CRMailAccountAnalyzer* _analyzer;
	id<CNCancelable> _listener;
	int _lock;

}
-(id)senderEmailAddresses;
-(char)isSyncingDisabledForAccountWithAddress:(id)arg1 ;
-(void)resetCachedAccountInfo;
-(void)runWithAnalyzerLock:(/*^block*/id)arg1 ;
-(id)analyzeMailAccounts;
-(void)dealloc;
-(id)init;
@end

