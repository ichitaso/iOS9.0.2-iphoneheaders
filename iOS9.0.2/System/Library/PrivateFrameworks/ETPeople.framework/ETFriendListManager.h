/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:32:36 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/PrivateFrameworks/ETPeople.framework/ETPeople
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <ETPeople/ETFriendGroupDelegate.h>
#import <libobjc.A.dylib/IDSBatchIDQueryControllerDelegate.h>

@protocol OS_dispatch_queue;
@class NSMutableArray, NPSManager, NPSDomainAccessor, NSTimer, NSObject, NSMutableDictionary, NSHashTable, NSMapTable, NSMutableSet, NSArray, IDSBatchIDQueryController, ETPerson, NSString;

@interface ETFriendListManager : NSObject <ETFriendGroupDelegate, IDSBatchIDQueryControllerDelegate> {

	NSMutableArray* _friendList;
	NSMutableArray* _friendGroups;
	NPSManager* _npsManager;
	NPSDomainAccessor* _domainAccessor;
	NSTimer* _saveTimer;
	NSObject*<OS_dispatch_queue> _saveQueue;
	unsigned _postCount;
	NSMutableDictionary* _changeLog;
	NSHashTable* _ephemeralPersons;
	NSMapTable* _identifiersToPersonMap;
	char _needsFriendListSync;
	char _needsAddressBookRefresh;
	NSMutableSet* _highPriorityDestinations;
	NSArray* _pendingDestinations;
	NSMutableDictionary* _cachedStatuses;
	NSTimer* _idsQueryTimeoutTimer;
	IDSBatchIDQueryController* _queryController;
	NSMutableArray* _blockList;
	unsigned _blockListPostCount;
	char _refreshAgainstAddressBookDisabled;
	ETPerson* _activeEphemeralPerson;

}

@property (nonatomic,retain) ETPerson * activeEphemeralPerson;                                                               //@synthesize activeEphemeralPerson=_activeEphemeralPerson - In the implementation block
@property (assign,getter=isRefreshAgainstAddressBookDisabled,nonatomic) char refreshAgainstAddressBookDisabled;              //@synthesize refreshAgainstAddressBookDisabled=_refreshAgainstAddressBookDisabled - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(id)sharedInstance;
+(char)_skipHasGizmoCheck;
+(char)_isRetailDemo;
+(id)collapseChangeLogsIntoChangeLog:(id)arg1 ;
-(void)save;
-(void)idStatusUpdatedForDestinations:(id)arg1 ;
-(void)dealloc;
-(id)init;
-(int)_firstEmptyPosition;
-(void)friendGroup:(id)arg1 didRemoveFriend:(id)arg2 atPosition:(unsigned)arg3 ;
-(void)friendGroup:(id)arg1 didSetFriend:(id)arg2 atPosition:(unsigned)arg3 ;
-(void)friendGroup:(id)arg1 didMoveFriends:(id)arg2 ;
-(char)addFriend:(id)arg1 ;
-(void)_personValuesChanged:(id)arg1 ;
-(void)_unreadCountChanged:(id)arg1 ;
-(void)_createEmptyFriendList;
-(void)_loadFriendList;
-(void)_loadGroupTitles;
-(void)_loadBlockList;
-(void)_postGroupListChangedNotification;
-(void)_addEmptyGroup;
-(void)_updateFriends:(id)arg1 ;
-(void)_deduplicateFriendList;
-(void)markFriendListAsNormalized;
-(id)_curatedFriendList;
-(void)_addCuratedFriends:(id)arg1 ;
-(void)_createAddressToPersonDictionary;
-(void)_updateFriendGroups;
-(unsigned)_numberOfFriendsInList:(id)arg1 ;
-(void)_aggdLogFriendGroupCount;
-(void)saveFriendGroupTitles;
-(char)_canAddFriendGroup;
-(char)_shouldAddEmptyGroup;
-(void)_removeGroupAtIndex:(unsigned)arg1 ;
-(unsigned)_indexForPosition:(unsigned)arg1 inGroup:(id)arg2 ;
-(void)_addEntryToChangeLogForPerson:(id)arg1 changeType:(id)arg2 ;
-(unsigned)groupIndexContainingFriend:(id)arg1 ;
-(id)_friendAtPosition:(unsigned)arg1 ;
-(void)_save:(id)arg1 ;
-(void)_writeToUserDefaults:(id)arg1 synchronize:(char)arg2 ;
-(void)_storeEphemeralPerson:(id)arg1 ;
-(void)_addPersonAddressesToDictionary:(id)arg1 ;
-(id)_destinations;
-(void)_initiateIDSDestinationStatusQuery:(id)arg1 ;
-(void)_stopIDSQueryTimeoutTimer;
-(int)_compareStatus:(int)arg1 toStatus:(int)arg2 ;
-(void)_queryDestinations:(id)arg1 ;
-(void)_setupQueryController;
-(void)_startIDSQueryTimeoutTimer;
-(void)_idsQueryTimeoutTimerFired;
-(void)refreshAgainstAddressBook;
-(char)_changeLogContainsFriendAdditionsOrUpdates;
-(int)_lastKnownAddressBookSequenceNumber;
-(char)_addressBookSequenceNumberDidChange;
-(void)_updateLastKnownAddressBookSequenceNumber:(int)arg1 ;
-(void)_postBlockListChangeNotification;
-(void)_saveBlockList;
-(char)_writeBlockListToUserDefaults:(id)arg1 ;
-(id)friendGroups;
-(char)canAddFriend;
-(void)syncFriendGroup:(id)arg1 ;
-(void)purgeEmptyFriendGroups;
-(char)isPersonFriend:(id)arg1 ;
-(unsigned)positionOfFriendInGroup:(id)arg1 ;
-(char)containsFriendWithABRecordGUID:(id)arg1 ;
-(id)personLikePerson:(id)arg1 ;
-(id)personWithDestinations:(id)arg1 ;
-(id)personWithABRecordGUID:(id)arg1 ;
-(void)refreshDestinationStatuses;
-(int)statusForPerson:(id)arg1 requery:(char)arg2 ;
-(void)_friendListChangedExternally;
-(void)_groupTitleChangedExternally;
-(void)setRefreshAgainstAddressBookDisabled:(char)arg1 ;
-(void)blockAddresses:(id)arg1 ;
-(void)unblockAddresses:(id)arg1 ;
-(char)isAddressBlocked:(id)arg1 ;
-(void)_blockListChangedExternally;
-(ETPerson *)activeEphemeralPerson;
-(void)setActiveEphemeralPerson:(ETPerson *)arg1 ;
-(char)isRefreshAgainstAddressBookDisabled;
-(id)blockList;
-(void)_addressBookChanged:(id)arg1 ;
-(id)allPeople;
-(void)_postChangeNotification;
@end

