/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:33:19 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/PrivateFrameworks/Message.framework/Message
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <Message/Message-Structs.h>
#import <libobjc.A.dylib/NSCopying.h>

@class NSString, MFWeakReferenceHolder, NSMutableArray, MailAccount, MFMessageCriterion, NSArray, NSNumber, NSMutableDictionary;

@interface MFMailboxUid : NSObject <NSCopying> {

	NSString* uniqueId;
	unsigned _mailboxID;
	MFWeakReferenceHolder* _account;
	NSString* _pathComponent;
	unsigned _attributes;
	MFWeakReferenceHolder* _parent;
	NSMutableArray* _children;
	MailAccount* _representedAccount;
	MFMessageCriterion* _criterion;
	char allCriteriaMustBeSatisfied;
	NSString* _realFullPath;
	NSArray* _extraAttributes;
	NSNumber* _pendingLevel;
	int _type;
	NSMutableDictionary* _userInfo;
	NSString* _permanentTag;

}

@property (nonatomic,retain) NSArray * extraAttributes;                                       //@synthesize extraAttributes=_extraAttributes - In the implementation block
@property (nonatomic,retain) NSString * permanentTag;                                         //@synthesize permanentTag=_permanentTag - In the implementation block
@property (nonatomic,readonly) unsigned suggestionsLostMessageSearchResultCount; 
@property (nonatomic,readonly) double suggestionsLostMessageSearchTimestamp; 
+(id)iconFromBundle:(id)arg1 forType:(int)arg2 ;
+(id)shorcutIconNameForMailboxType:(int)arg1 ;
+(id)specialNameForType:(int)arg1 ;
+(char)isSpecialMailboxUidType:(int)arg1 ;
+(char)isOutgoingMailboxType:(int)arg1 ;
+(char)isDraftsMailboxType:(int)arg1 ;
+(char)isSentMailboxType:(int)arg1 ;
-(int)level;
-(id)iconFromBundle:(id)arg1 ;
-(void)setExtraAttributes:(NSArray *)arg1 ;
-(id)store;
-(NSArray *)extraAttributes;
-(char)isContainer;
-(void)dealloc;
-(id)init;
-(id)description;
-(void)setName:(id)arg1 ;
-(id)name;
-(void)invalidate;
-(char)isValid;
-(void)setType:(int)arg1 ;
-(int)type;
-(id)copyWithZone:(NSZone*)arg1 ;
-(id)URL;
-(char)isVisible;
-(void)removeChild:(id)arg1 ;
-(id)uniqueId;
-(unsigned)attributes;
-(id)displayName;
-(void)setAttributes:(unsigned)arg1 ;
-(id)URLString;
-(char)isSelectable;
-(unsigned)unreadCount;
-(id)accountRelativePath;
-(unsigned)serverUnreadOnlyOnServerCount;
-(id)representedAccount;
-(char)shouldRestoreMessagesAfterFailedDelete;
-(id)lastViewedMessageID;
-(id)lastViewedMessageDate;
-(unsigned)unreadCountMatchingCriterion:(id)arg1 ;
-(id)tildeAbbreviatedPath;
-(id)mutableCopyOfChildren;
-(id)depthFirstEnumerator;
-(void)flushCriteria;
-(char)setUnreadCount:(unsigned)arg1 ;
-(id)childWithName:(id)arg1 ;
-(id)childWithExtraAttribute:(id)arg1 ;
-(id)childWithPermanentTag:(id)arg1 ;
-(char)setChildren:(id)arg1 ;
-(void)setRepresentedAccount:(id)arg1 ;
-(id)initWithName:(id)arg1 attributes:(unsigned)arg2 forAccount:(id)arg3 permanentTag:(id)arg4 ;
-(id)initWithName:(id)arg1 attributes:(unsigned)arg2 forAccount:(id)arg3 extraAttributes:(id)arg4 ;
-(id)pathRelativeToMailbox:(id)arg1 ;
-(NSString *)permanentTag;
-(unsigned)suggestionsLostMessageSearchResultCount;
-(double)suggestionsLostMessageSearchTimestamp;
-(id)_initWithName:(id)arg1 attributes:(unsigned)arg2 forAccount:(id)arg3 ;
-(id)_mutableChildren;
-(id)childEnumeratorIncludingHiddenChildren:(char)arg1 ;
-(unsigned)numberOfDescendants;
-(id)URLStringNonNil;
-(id)fullPathNonNil;
-(char)isStore;
-(id)topMailbox;
-(id)rootMailbox;
-(id)ancestralAccount;
-(id)criterion;
-(id)_loadUserInfo;
-(char)mergeWithUserInfo:(id)arg1 ;
-(id)URLStringWithAccount:(id)arg1 ;
-(void)addToPostOrderTraversal:(id)arg1 ;
-(id)displayNameUsingSpecialNames;
-(unsigned)nonDeletedCount;
-(char)hasChildren;
-(id)childEnumerator;
-(unsigned)numberOfChildren;
-(unsigned)indexOfChild:(id)arg1 ;
-(id)descendantWithExtraAttribute:(id)arg1 ;
-(id)descendantWithPermanentTag:(id)arg1 ;
-(void)sortChildren;
-(char)isSpecialMailboxUid;
-(char)isOutgoingMailboxUid;
-(id)realFullPath;
-(id)pathRelativeToMailboxForDisplay:(id)arg1 ;
-(id)oldURLString;
-(unsigned)mailboxID;
-(int)compareWithMailboxUid:(id)arg1 ;
-(int)indexToInsertChildMailboxUid:(id)arg1 ;
-(char)isDescendantOfMailbox:(id)arg1 ;
-(char)userInfoBoolForKey:(id)arg1 ;
-(void)setUserInfoBool:(char)arg1 forKey:(id)arg2 ;
-(id)userInfoDictionary;
-(void)setUserInfoWithDictionary:(id)arg1 ;
-(id)userInfoForSerialization;
-(void)setCriterion:(id)arg1 ;
-(char)alwaysWriteFullMessageFile;
-(void)setLastViewedMessageID:(id)arg1 ;
-(void)updateSuggestionsLostMessageSearchResultCount:(unsigned)arg1 ;
-(void)setPermanentTag:(NSString *)arg1 ;
-(id)childAtIndex:(unsigned)arg1 ;
-(id)parent;
-(id)children;
-(void)setParent:(id)arg1 ;
-(id)userInfoObjectForKey:(id)arg1 ;
-(void)setUserInfoObject:(id)arg1 forKey:(id)arg2 ;
-(void)saveUserInfo;
-(id)fullPath;
-(id)initWithAccount:(id)arg1 ;
-(id)account;
@end

