/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:33:51 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/PrivateFrameworks/PowerlogCore.framework/PowerlogCore
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <PowerlogCore/PLEntry.h>

@class NSString, NSDate;

@interface PLArchiveEntry : PLEntry {

	char _systemTimeOffsetModified;

}

@property (nonatomic,readonly) NSString * path; 
@property (nonatomic,readonly) NSString * compressedPath; 
@property (nonatomic,readonly) NSString * crPath; 
@property (nonatomic,readonly) NSString * name; 
@property (readonly) NSDate * startDate; 
@property (readonly) NSDate * endDate; 
@property (readonly) NSString * uuid; 
@property (assign,nonatomic) double systemTimeOffset; 
@property (readonly) char systemTimeOffsetModified;                    //@synthesize systemTimeOffsetModified=_systemTimeOffsetModified - In the implementation block
@property (readonly) char fullMode; 
@property (assign,nonatomic) int stage; 
@property (assign,nonatomic) unsigned numAttempts; 
@property (nonatomic,retain) NSDate * syncedOffDate; 
@property (nonatomic,retain) NSDate * removedDate; 
@property (assign,nonatomic) long long mainDBSizeAtStart; 
@property (nonatomic,readonly) char syncedOff; 
@property (nonatomic,readonly) char removed; 
+(void)load;
+(id)storageQueue;
+(id)entryKey;
-(NSDate *)startDate;
-(NSDate *)endDate;
-(NSString *)name;
-(NSString *)path;
-(void)log;
-(int)stage;
-(void)setStage:(int)arg1 ;
-(NSString *)uuid;
-(double)systemTimeOffset;
-(NSString *)compressedPath;
-(unsigned)numAttempts;
-(NSDate *)syncedOffDate;
-(NSDate *)removedDate;
-(long long)mainDBSizeAtStart;
-(char)syncedOff;
-(NSString *)crPath;
-(id)initWithStartDate:(id)arg1 endDate:(id)arg2 andUUID:(id)arg3 ;
-(void)setSystemTimeOffset:(double)arg1 ;
-(void)setNumAttempts:(unsigned)arg1 ;
-(void)setSyncedOffDate:(NSDate *)arg1 ;
-(void)setRemovedDate:(NSDate *)arg1 ;
-(char)removed;
-(void)setMainDBSizeAtStart:(long long)arg1 ;
-(void)linkToCR;
-(char)systemTimeOffsetModified;
-(char)fullMode;
-(id)initEntryWithRawData:(id)arg1 ;
-(id)initWithMetadata:(id)arg1 ;
@end

