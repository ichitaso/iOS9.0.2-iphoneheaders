/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 6, 2015 at 2:34:10 PM Japan Standard Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /System/Library/PrivateFrameworks/SearchUI.framework/SearchUI
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <ContactsUI/CNQuickActionsView.h>
#import <libobjc.A.dylib/CNQuickActionsViewDelegate.h>

@protocol SearchUIResult;
@class SearchUITableViewCell, NSString;

@interface SearchUIContactActionButton : CNQuickActionsView <CNQuickActionsViewDelegate> {

	SearchUITableViewCell* _cell;
	id<SearchUIResult> _result;

}

@property (assign,nonatomic,__weak) SearchUITableViewCell * cell;              //@synthesize cell=_cell - In the implementation block
@property (nonatomic,retain) id<SearchUIResult> result;                        //@synthesize result=_result - In the implementation block
@property (readonly) unsigned hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(id)viewControllerForActionsView:(id)arg1 ;
-(void)actionsView:(id)arg1 didPerformAction:(id)arg2 ;
-(SearchUITableViewCell *)cell;
-(void)setCell:(SearchUITableViewCell *)arg1 ;
-(id<SearchUIResult>)result;
-(id)initWithContact:(id)arg1 style:(unsigned)arg2 cell:(id)arg3 result:(id)arg4 ;
-(id)initWithActionButtonItem:(id)arg1 style:(unsigned)arg2 cell:(id)arg3 result:(id)arg4 ;
-(void)setResult:(id<SearchUIResult>)arg1 ;
@end

